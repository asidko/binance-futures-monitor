...
- Conditions are given as compound tokens, expanded by `conditions.resolve_conditions(tokens, price, level, candle)`: a bare type (`closed`/`crosses`) auto-picks its direction (below level -> `*above`, at/above -> `*below`); a bare direction (`above`/`below`) takes both types; `closed-opposite` takes the color opposite the last closed candle (-> `closed-red`/`closed-green`); a full name passes through. Concrete names are `<type>-<qualifier>` where qualifier is a direction (`above`/`below`) or a color (`red`/`green`); color conditions fire on a close of that color and ignore the level. Omit `--condition` to use `default_condition` (config, comma-separated) else both types auto. Resolved to concrete names at add-time (CLI fetches the price only for a bare type, the last candle only for `closed-opposite`), so the store always holds real condition names - no placeholder mode.
